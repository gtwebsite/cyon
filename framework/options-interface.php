...
function optionsframework_fields() {
	...
	foreach ( $options as $value ) {
		...
		$class = '';
	 	...
	 	// Added section_start and end for grouping  by Gtwebsite - MODIFIED
		...
		if ( ( $value['type'] != "heading" ) && ( $value['type'] != "info" ) ) {
			...
			if( $value['type'] != 'section_start' ){  // Added section_start by Gtwebsite - MODIFIED
		...
		// Textarea that does not sanitize at all, added by Gtwebsite - MODIFIED
		case 'texthtml':
			$cols = '8';
			$ta_value = '';
		...
		case "heading":
			...
			//$output .= '<h3>' . esc_html( $value['name'] ) . '</h3>' . "\n";  // Not needed anymore because of sections by Gtwebsite - MODIFIED
			break;
		}

		if ( ( $value['type'] != "heading" ) && ( $value['type'] != "info" ) && $value['type'] != 'section_start' ) { // Added section_start by Gtwebsite - MODIFIED
			$output .= '</div>';
